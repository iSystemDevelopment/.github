# iSystem Development — documentation index

Contributor docs for the **iSystemDevelopment** GitHub organization.

| Surface | URL |
|---------|-----|
| Products | [isystem.app](https://isystem.app) |
| Private SSOT | `isystem-codebase` (org members) |

---

## Start here

| Document | Purpose |
|----------|---------|
| [ONBOARDING.md](ONBOARDING.md) | Access, repos, first session |
| [ENGINEERING_PRINCIPLES.md](ENGINEERING_PRINCIPLES.md) | Quality bar, no slop |
| [AI_ORCHESTRATION.md](AI_ORCHESTRATION.md) | AI session workflow + change reports |
| [WORKFLOW.md](WORKFLOW.md) | Branches, review, deploy |
| [SEO_ROCKET.md](SEO_ROCKET.md) | Meta, OG, icons, social launch bundle |
| [VISUAL_CONSISTENCY.md](VISUAL_CONSISTENCY.md) | Dark UI, mobile, TIP chrome |
| [ACCESS_CONTROL.md](ACCESS_CONTROL.md) | Permissions (leads) |
| [FAQ.md](FAQ.md) | Common questions |

---

## Templates & license

| Path | Purpose |
|------|---------|
| [../templates/CONTRIBUTING.md](../templates/CONTRIBUTING.md) | Contributions |
| [../templates/SECURITY.md](../templates/SECURITY.md) | Security reporting |
| [../templates/PULL_REQUEST_TEMPLATE.md](../templates/PULL_REQUEST_TEMPLATE.md) | PR template |
| [../workflows/README.md](../workflows/README.md) | CI |
| [../LICENSE.md](../LICENSE.md) | Org proprietary license |

---

## Wiki

Source: [`../wiki/`](../wiki/) — sync to GitHub Wiki only if enabled.

---

*Updated 2026-07-06*
